...
def mmt(x, N,T,data):
    ...
    iota = 0.001
    soft_penalty_param = 1e8
    FF = jnp.array(0.0)

    FF += soft_penalty_param * 1/(1 + jnp.exp(-(mean_sr - 0.36)/iota))
    ...
    FF += soft_penalty_param * 1/(1 + jnp.exp(jnp.abs(-dApibar[0] - 0.25 / 100)/iota))
    ...
    FF += soft_penalty_param * 1/(1 + jnp.exp(-(Api[-1] - (0.01))/iota))
